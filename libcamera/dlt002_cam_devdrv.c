...
static int32_t dlt002_check_bsts(uint8_t value, uint32_t timeout);
...
static int32_t dlt002_check_msts(uint8_t value, uint32_t timeout);
...
static int32_t dlt002_monitor_config()
{
	...
	ret = dlt002_check_msts(DLT002_MSTS_MON_VAL, DLT002_POLLING_TIMES);
	if (ret) {
...
static int32_t dlt002_raw_snapshot_config()
{
	...
	if (dlt002_ctrl->scene == SENSOR_SCENE_TWILIGHT) {
		ret = dlt002_check_bsts(DLT002_BSTS_VAL, DLT002_POLLING_TIMES);
		if (ret) {
